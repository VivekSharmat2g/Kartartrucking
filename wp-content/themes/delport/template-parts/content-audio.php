...
$delport_audio_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
...
?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('ablog ablog-4 mb-60 format-audio'); ?>>
        <?php if (!empty($delport_audio_url)) : ?>
            <div class="postbox__audio embed-responsive embed-responsive-16by9 position-relative">
                <?php echo wp_oembed_get($delport_audio_url); ?>
            </div>
        <?php endif; ?>
...
<?php else : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('bd-blog mb-60 format-audio'); ?> data-wow-delay=".3s">
        <?php if (!empty($delport_audio_url)) : ?>
            <div class="postbox__audio embed-responsive embed-responsive-16by9 position-relative">
                <?php echo wp_oembed_get($delport_audio_url); ?>
            </div>
