...
function bdevs_custom_post_types() {
    $eurtech_sv_name = get_theme_mod('eurtech_sv_name','Services');
    $eurtech_sv_slug = get_theme_mod('eurtech_sv_slug','ourservices');

    $eurtech_shop_name = get_theme_mod('eurtech_shop_name','Portfolio');
    $eurtech_shop_slug = get_theme_mod('eurtech_shop_slug','ourportfolio');

    return [
        'bdevs-services' => array('title' => 'Service', 'plural_title' => $eurtech_sv_name, 'rewrite' => $eurtech_sv_slug,'menu_icon' => 'dashicons-admin-generic'),
        'bdevs-portfolio'    => [ 'title' => 'Portfolio', 'plural_title' => $eurtech_shop_name, 'rewrite' => $eurtech_shop_slug, 'menu_icon' => 'dashicons-format-image' ],
    ];
}
...
// Load textdomain
function eurtech_load_plugin_textdomain() {
    load_plugin_textdomain( 'bdevs-toolkit', false, basename( dirname( __FILE__ ) ) . '/languages/' );
}
add_action( 'init', 'eurtech_load_plugin_textdomain' );