(function ($) {
	...
	$(document).ready(function () {
		...
		//jQuery('input[id="ultn_params[background_image]"]').trigger('change');
		...
		$("body").on(
			"click",
			".nx-metatab-menu li, .utl-notifications-builder-tab-menu li, .nx-meta-next, .nx-quick-builder-btn",
			function (e) {
				e.preventDefault();
		...
		$(".nx-on-click-install").each(function (e) {
			$(this).on("click", function (e) {
				...
				$.ajax({
					...
					type: "POST",
					data: {
						action: "unlimited_notifications_upsale_core_install_notificationx",
						_wpnonce: nonce,
						slug: slug,
						file: plugin_file,
					},
	...
	$(window).load(function () {
		...
		if (qVars != undefined) {
			if (qVars.indexOf("utl-notifications-settings") >= 0) {
				var cSettingsTab = qVars.split("#");
				$(
					'.utl-notifications-settings-menu li[data-tab="' + cSettingsTab[1] + '"]'
				).trigger("click");
			}
		}
		...
		$("body").on(
			"change",
			".utl-notifications-builder-content-wrapper .nx_meta_display_type",
			function (e) {
	...
	$.notificationx.create_nx_bar = function () {
		$('body').on('change', '.nx_meta_display_type', function () {
			var type = $(this).val();
			if (type == 'press_bar' && $('.nx-bar_with_elementor-remove').length > 0) {
				$('#nx-meta-close_forever_2').show().find('input').attr('name', 'nx_meta_close_forever_3');
			}
			else {
				$('#nx-meta-close_forever_2').hide();
			}
		});
	...
	$.notificationx.bindEvents = function () {
		 ...
		 * Settings Tab
		 */
		$("body").delegate(".utl-notifications-settings-menu li", "click", function (e) {
			$.notificationx.settingsTab(this);
		});

		var saveButton = $(".utl-notifications-settings-button");
		...
		$(
			".nx-checkbox-area .nx-checkbox input:enabled, .utl-notifications-settings-field"
		).on("click", function (e) {
			saveButton
		...
		$("body").delegate(".utl-notifications-settings-button", "click", function (e) {
			e.preventDefault();
			var form = $(this).parents("#utl-notifications-settings-form");
	...
	$.notificationx.initializeFields = function () {
		if ($(".nx-meta-field, .utl-notifications-settings-field").length > 0) {
			$(".nx-meta-field, .utl-notifications-settings-field").map(function (
				iterator,
				item
			) {
		...
		$(".utl-notifications-settings .utl-notifications-settings-field").trigger("change");
	...
	$.notificationx.tabChanger = function (buttonName) {
		...
		var contentMenu = $(".utl-notifications-builder-tab-menu").find(
		...
		if (contentMenu.length > 0) {
			if (contentMenu[0] != undefined) {
				...
				var lMenu = $(
					'.utl-notifications-builder-tab-menu li[data-tabid="' + tabID + '"]'
				);
		...
		$('.utl-notifications-builder-tab-menu li[data-tabid="' + tabID + '"]').trigger(
			"click"
		);
	};
	...
	$.notificationx.toggleFields = function () {
		$("body").delegate(
			".nx-meta-field, .utl-notifications-settings-field",
			"change",
			function (e) {
				if (this.type == "radio") {
					if (this.checked) {
